...
class Attacker:
    ...
    def _attack(self):
        ...
        if self._checkpoint:
            ...
            num_timedout = self._checkpoint.num_timedout_attacks
        else:
            num_results = 0
            num_failures = 0
            num_skipped = 0
        ...
        while worklist:
            ...
            if isinstance(result, FailedAttackResult):
                num_failures += 1
            if isinstance(result, TimedoutAttackResult):
                num_timedout += 1

            pbar.set_description(
                f"[Succeeded / Failed / Skipped / Timedout / Total] {num_successes} / {num_failures} / {num_skipped} / {num_timedout} / {num_results}"
            )
