...
def timeout(seconds=None, error_message="Function call timed out"):
    def decorator(func):
        def _handle_timeout(signum, frame):
           raise AlarmException(error_message)

        @wraps(func)
        def wrapper(*args, **kwargs):
            if seconds is not None:  # Only set the alarm if seconds is specified
            ...
            try:
                ...
                if seconds is not None:
                    signal.alarm(0)  # Cancel the alarm if it was set
            return result
...
class Attack:
    ...
    def search_method_with_timeout(self, initial_result):
        decorator = timeout(seconds=self.time_budget)(self.search_method)
        return decorator(initial_result)
    ...
    def _attack(self, initial_result):
        ...
        """
        try:
            final_result = self.search_method_with_timeout(initial_result)
        except Exception as e:  # Catching broadly here
            current_exception = e
            alarm_exception_detected = False
            while current_exception:
                if isinstance(current_exception, AlarmException):
                    alarm_exception_detected = True
                    break
                current_exception = current_exception.__cause__ or current_exception.__context__
            
            if alarm_exception_detected:
                # Handle the case where AlarmException was detected in the chain
                return TimedoutAttackResult(initial_result, None)
            else:
                # Re-raise the original exception if it's not related to AlarmException
                raise
